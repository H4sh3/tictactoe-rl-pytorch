...
def play_training_games(net_context, qplayer, total_games, discount_factor,
    ...
    for game in range(total_games):
        ...
        if not x_strategies:
            x = [create_training_player(
                net_context, move_history, epsilon)]
            x_strategies_to_use = itertools.cycle(x)

        if not o_strategies:
            o = [create_training_player(
                net_context, move_history, epsilon)]
...
def update_training_gameover(net_context, move_history, q_learning_player,
    ...
    backpropagate(net_context, next_position,
                  move_index, game_result_reward)

    for (position, move_index) in list(move_history)[1:]:
        with torch.no_grad():
            next_q_values = get_q_values(
                next_position, net_context.target_net)
            qv = torch.max(next_q_values).item()

        backpropagate(net_context, position, move_index,
                      discount_factor * qv)
